const appConfigs = {
    cube: {
        title: 'Rotating cube',
        icon: './images/appIcons/cube.png',
        iframeSrc: 'https://fireroth.is-a.dev/sottau/webGLCube/',
        windowSize: {width: 900, height: 650},
        minSize: {width: 300, height: 200},
        resizable: true
    },
    about: {
        title: 'JScript OS',
        icon: './images/appIcons/info.png',
        iframeSrc: './apps/about/',
        windowSize: {width: 350, height: 270},
        minSize: {width: 350, height: 260},
        resizable: false
    },
    notepad: {
        title: 'Notepad',
        icon: './images/appIcons/notepad.png',
        iframeSrc: './apps/notepad/',
        windowSize: {width: 500, height: 350},
        minSize: {width: 200, height: 150},
        resizable: true
    },
    error: {
        title: 'Error',
        icon: './images/appIcons/error.png',
        iframeSrc: './apps/error/',
        windowSize: {width: 470, height: 180},
        minSize: {width: 400, height: 150},
        resizable: false
    },
    devPanel: {
        title: 'Developer stuff',
        icon: './images/appIcons/code.png',
        iframeSrc: './apps/devPanel/',
        windowSize: {width: 700, height: 450},
        minSize: {width: 700, height: 200},
        resizable: true
    },
    console: {
        title: 'JS Console',
        icon: './images/appIcons/console.png',
        iframeSrc: './apps/console/',
        windowSize: {width: 500, height: 300},
        minSize: {width: 300, height: 150},
        resizable: true
    },
    settings: {
        title: 'Settings',
        icon: './images/appIcons/settings.png',
        iframeSrc: './apps/settings/',
        windowSize: {width: 650, height: 420},
        minSize: {width: 500, height: 250},
        resizable: true
    },
    run: {
        title: 'Run',
        icon: './images/appIcons/program.png',
        iframeSrc: './apps/run/',
        windowSize: {width: 450, height: 230},
        minSize: {width: 300, height: 230},
        resizable: false
    },
    allApps: {
        title: 'All Apps',
        icon: './images/appIcons/folder.png',
        iframeSrc: './apps/allApps/',
        windowSize: {width: 500, height: 300},
        minSize: {width: 300, height: 150},
        resizable: true
    },
    calculator: {
        title: 'Calculator',
        icon: './images/appIcons/calculator.png',
        iframeSrc: './apps/calculator/',
        windowSize: {width: 300, height: 410},
        minSize: {width: 200, height: 350},
        resizable: true
    },
    browser: {
        title: 'Web Browser',
        icon: './images/appIcons/browser.png',
        iframeSrc: './apps/webBrowser/',
        windowSize: {width: 650, height: 420},
        minSize: {width: 500, height: 250},
        resizable: true
    }
};